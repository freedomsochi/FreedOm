...
(function(){
  ...
  window.FreeDomGuestStatus = {
    ...
    booking(b){
      const total=num(b?.total_amount), paid=num(b?.paid_amount);
      return this.payment(paid,total,Math.max(0,total-paid));
    },
    order(o){
      const total=num(o?.total_amount), paid=num(o?.paid_amount);
      return this.payment(paid,total,Math.max(0,total-paid));
    },
    chip(s){
      return `<span class="chip ${s.className}">${s.icon} ${s.label}</span>`;
    }
  };
})();
