...
(function(){
  ...
  async function act(id,action,btn){
    ...
    }catch(e){alert(e.message||'Не удалось изменить заявку');btn.disabled=false;btn.textContent=old}
  }
  function enhance(){
    ...
    cards.forEach(card=>{
      ...
      const yes=document.createElement('button');yes.className='booking-confirm';yes.textContent='✓ Подтвердить';yes.onclick=()=>act(id,'confirm',yes);
      const no=document.createElement('button');no.className='booking-cancel';no.textContent='× Отменить';no.onclick=()=>act(id,'cancel',no);
      box.append(label,yes,no);card.appendChild(box);
    });
    const pending=cards.filter(c=>['новое','new','pending'].includes(c.dataset.bookingStatus));
    if(pending.length)pending.reverse().forEach(card=>list.prepend(card));
  }
